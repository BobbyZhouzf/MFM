...
def match_tp_of_two_series(series1=None, series2=None, indicator1=None, indicator2=None,
    ...
    for dt1, tp1 in indicator1.iloc[no_data:].iteritems():
        ...
        if tp1 == latest_tp2:
            if ((dt1 - lastet_dt2) / pd.Timedelta(1, 'D') >= look_back_cache * freq_scaler) or (len(indicator1.loc[lastet_dt2:dt1]) > 1):
                # continue
                matched += 1
            ...
            idx = np.where(
                indicator2.index <= dt1+pd.Timedelta(int(lookforward_cache*freq_scaler), 'D'))[0][-1]
            latest_tp2 = indicator2.iat[idx]
            lastet_dt2 = indicator2.index[idx]
            if tp1 == latest_tp2:
                matched += 1
...
'''Test'''
PMI=pd.read_excel('D:\Python\MacroFactorModel\PMI.xlsx',index_col=0,header=0,sheet_name='PMI')['PMI']
CC=pd.read_excel('D:\Python\MacroFactorModel\PMI.xlsx',index_col=0,header=0,sheet_name='PMI')['CC']
PMI_Detect=detect_tp_of_time_series(PMI, minimal_cycle_length=15, minimal_phase_length=5, log_file=None)
CC_Detect=detect_tp_of_time_series(CC, minimal_cycle_length=15, minimal_phase_length=5, log_file=None)

Result,leading_periods=match_tp_of_two_series(series1=PMI, series2=CC, indicator1=None, indicator2=None,
                           freq_scaler=28, minimal_cycle_length1=15, minimal_cycle_length2=15,
                           minimal_phase_length1=5, minimal_phase_length2=5, log_file=None,
                           look_back_cache=0, lookforward_cache=12, return_details=False)
#
# A=pd.read_excel('D:\Python\MacroFactorModel\PMI.xlsx',index_col=0,header=0,sheet_name='Peak').dropna()['A']
# B=pd.read_excel('D:\Python\MacroFactorModel\PMI.xlsx',index_col=0,header=0,sheet_name='Peak').dropna()['B']

compare_two_series(PMI, CC, PMI_Detect, CC_Detect, save_plots=False, show_plots=True,
                       matched_periods=None)
# series1=PMI
# series2=CC
